...
Given that diagonal values of $\textbf{M}$ are set to $-1$, We can know the relationship $S = 1600$, $C = 1$, and $\sigma = 1/40$. Under these parameter values, given $\rho = 0$, we know that $\sigma\sqrt{SC}\left(1 + \rho\right) = 1$. Hence, the expected value of $\max(\Re(\lambda)) = 0$
...
Hence, under the same set of parameters, we predict a linear effect of $\rho$ on $\max(\Re(\lambda)) = 0$, such that $\max(\Re(\lambda)) = \rho$. 

Allesina and Tang note that $\rho$ is defined as follows [@Allesina2015a],
