...
Variation in $\gamma$ ($\sigma^{2}_{\gamma}$) **can increase or decrease the radius of the circle**.
...
<!--- still wrong below -- no simple analytical solution seems possible --->
...
$$2 \sigma_{\gamma} \sigma_{\textbf{A}} \sqrt{SC}.$$
...
For poisson, it's $$\sigma^{2}_{\gamma} \sigma_{\textbf{A}} \sqrt{SC}.$$

The mean of a uniform is $(1/2)b$, while the mean of the Poisson is $\lambda = \sigma^{2}_{\gamma}$



Note that the error around the expected radius of eigenvalues is not normally distributed. It is log distributed (check).
