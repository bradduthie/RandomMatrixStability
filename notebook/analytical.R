...
for(i in 1:100000){
    ...
    A0_new_dat <- rnorm(n = Snew, mean = 0, sd = sigma);
    A0_new_mat <- matrix(data = A0_new_dat, nrow = Snew, ncol = Snew);
    gammas_new <- runif(n = 256, min = 0, max = 2);#c(rep(0.05, Snew/2), rep(1.95, Snew/2));
    mu_gam_new <- mean(gammas_new);
    diag(A0_new_mat) <- -1 * dval;
    A1_new    <- gammas_new * A0_new_mat;
    A0_new    <- mu_gam_new * A0_new_mat;
...
###### USE THIS -- ODD THAT IT WORKS.

# TRY TO REASON, IF APPROACH OF DISTRIBUTION FROM \S TO INF IS FASTER GIVEN
# MORE VARIATION AROUND L? THIS DOESN'T SEEM TO WORK.


S        <- 400;
sigma    <- 0.4 # 1 / sqrt(S);
iseq     <- seq(from = -2, to = 2, by = 0.01);
rseq     <- seq(from = -4, to = 1, by = 0.01);
ttab     <- matrix(data = 0, nrow = length(iseq) * length(rseq), ncol = 4);
k        <- 1;
L        <- matrix(data = 0, nrow = S, ncol = S);
diag(L)  <- runif(n = S, min = 0, max = 2); # 
...
A0_new_dat <- rnorm(n = Snew, mean = 0, sd = sigma);
A0_new_mat <- matrix(data = A0_new_dat, nrow = Snew, ncol = Snew);
gammas_new <- runif(n = Snew, min = 0, max = 2);#c(rep(0.05, Snew/2), rep(1.95, Snew/2));
mu_gam_new <- mean(gammas_new);
diag(A0_new_mat) <- -1;
A1_new    <- gammas_new * A0_new_mat;
A0_new    <- mu_gam_new * A0_new_mat;
...
hist(A0_new_r, breaks = 30, col = "blue", xlim = c(min(A1_new_r), 0.5));
hist(A1_new_r, breaks = 30, add = TRUE, col = "red");
