...
Hence, in general, $Var(\gamma)$ does not appear to affect feasibility in pure species interaction networks [@Servan2018].
...
Stability results are also shown in the table below. Results for `A0` indicate systems in which $\gamma = 1$, while `A1` refers to systems in which the genetic algorithm searched for a set of $\gamma$ values that stabilised the system. 
...
```{r, echo = FALSE}
load("../data/constant_complexity.rda");
