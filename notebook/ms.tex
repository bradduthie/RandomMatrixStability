...
gave similar results). Diagonal elements of simulated \(\mathbf{A}\)
were standardised to the mean value of sampled \(\gamma\) so that
\(E[d]\) of \(\textbf{M}\) and \(\textbf{M}\) were guaranteed to be
identical. Here I focus on the effect of \(\gamma\) across values of
\(\rho\), and for increasing system sizes (\(S\)) in random and
structured networks. By increasing \(S\), the objective is to determine
the effect of \(\gamma\) as system complexity increases toward the
boundary at which stability is realistic for a finite system.
...
\(\sigma = 0.4\). For \(-0.4 \leq \rho \leq 0.7\) (shaded region of Fig.
2), expected \(\Re(\lambda_{max})\) was lower in \(\mathbf{M}\) than
\(\mathbf{A}\). For \(\rho \geq -0.1\), the lower bound of the range of
\(\Re(\lambda_{max})\) values also decreased given
...
simulations of \(\mathbf{M}\) across increasing \(S\) given \(C = 1\)
...
\(\sigma^{2}_{\gamma}\) never decreased the expected
...
systems of varying complexity, I simulated random
\(\mathbf{M = \gamma A}\) matrices at \(\sigma = 0.4\) and \(C = 1\)
...
\(\mathbf{A}\) (Fig. 3; see Supplementary Information for full table of
...
random networks that are typically interpreted as differing
interspecific ecological interaction
...
\(\mathbf{A}\) were observed for competitor and predator-prey, but not
mutualist, systems. A higher number of stable systems were observed
...
\(\sigma \sqrt{SC} \gtrapprox 1.26\). This threshold of complexity,
...
\(\sqrt{\sigma^{2}_{A}(1 + \sigma^{2}_{\gamma})S} \approx\) 1.21.
