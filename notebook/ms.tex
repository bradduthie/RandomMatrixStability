...
i.i.d \(A_{ij} \sim \mathcal{N}(0, 0.4)\) with a probability \(C\)
(unsampled elements were set to zero). Diagonal elements \(A_{ii}\) were
set to \(-1\). Elements of \(\gamma\) were simulated i.i.d. from a
distribution with positive support (typically
...
sampled i.i.d. from \(\mathcal{N}(0, 0.4)\). To build the interaction
matrix \(\mathbf{A}\), I used element-wise multiplication
\(\mathbf{A} = \mathbf{J} \odot \mathbf{B}\), then set
\(diag(\mathbf{A}) = -1\). I set \(k = S/2\) and simulated small-world
...
\(-1\). I simulated scale-free networks across all combinations of
\(S = \{24, 48, 72, 96, 120\}\) and \(m = \{2, 3, ..., 11, 12\}\).

Cascade food-webs were constructed following Solow and
Beet\textsuperscript{\protect\hyperlink{ref-Solow1998}{17}}. First, a
random matrix \(\mathbf{A}\) was generated with off-diagonal elements
sampled i.i.d so that \(A_{ij} \sim \mathcal{N}(0, 0.4)\). Each
...
cascade food webs across all combinations of
\(S = \{2, 3, ..., 59, 60\}\).
...
\(\sqrt{\sigma^{2}_{A}(1 + \sigma^{2}_{\gamma})S} \approx\) 1.11.
