...
therefore becomes increasingly unlikely as \(S_{M}\), \(C_{M}\), and
\(\sigma_{M}^{2}\) increase.
...
(diagonal elements all equal 1) and \(\textbf{M} = \textbf{A}\).

\textbf{Analytical considerations}. The value of \(\max(\Re(\lambda))\),
and therefore system stability, can be estimated from five properties of
\(\textbf{M}\)\textsuperscript{\protect\hyperlink{ref-Tang2014b}{19}}.
...
Variation of the off-diagonal elements in \(\textbf{M}\) is described by
...
\(\textbf{M}\) lie within a circle of a radius
\(\sigma^{2}_{A}\sqrt{S}\) and \(\sigma^{2}_{M}\sqrt{S}\), respectively
(Figure 1a vs.~1b). When components are self-regulating (e.g.,
\(d = -1\)), positive \(\sigma^{2}_\gamma\) changes the distribution of
eigenvalues, potentially affecting stability (Figure 1c vs.~1d).

Given \(\sigma^{2}_\gamma = 0\), \(\max(\Re(\lambda))\) decrease
linearly with \(\rho\) such that,
...
\textbf{Numerical simulations}. I used numerical simulations to isolate
the effect of \(\sigma_{\gamma}\) on the stability of
\(\textbf{M} = \gamma \textbf{A}\) by comparing \(\sigma_{\gamma} > 0\)
versus \(\sigma_{\gamma} = 0\). Values of \(\gamma_{ii}\) were sampled
such that \(E[d]\) and \(E[\mu]\) also remained unchanged (in practice,
diagonal elements of \(\textbf{M}\) were standardised so that mean
values were identical before and after adding \(\gamma\)).
...
Proportions of \(\Re(\lambda) < 0\) are 0.727 and 0.733 for a and b,
respectively.
