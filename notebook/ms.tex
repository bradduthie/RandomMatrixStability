...
\(\max(\Re(\lambda))\). But as \(S\) increased, the curvilinear
relationship between \(\rho\) and \(\max(\Re(\lambda))\) resulted in
\(\mathbf{M}\) with lower expected \(\max(\Re(\lambda))\) for low
