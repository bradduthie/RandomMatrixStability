...
otherwise be negligible. At such extremes of simulated system size, the
...
words, the potential for \(Var(\gamma)\) to drive stability increased
...
Information). I want to emphasise that \(\gamma\) is not interpreted as
...
that are especially large (but finite, see Gibbs et
al.\textsuperscript{\protect\hyperlink{ref-Gibbs2017}{17}}), and it can
ultimately increase the probability that system stability is observed
...
probability that the system will be stable. Rather, highly complex
systems that are observed to be stable are more likely to have varying
component response rates, and for this variation to be critical to their
stability (Fig. 3). This is caused by the shift to a non-uniform
distribution of eigenvalues that occurs by introducing \(Var(\gamma)\)
(Fig. 1b, 2b), which can sometimes cause all of the real components of
the eigenvalues of the community matrix \(\mathbf{M}\) to become
negative, but might also increase the real components of eigenvalues. In
this way, my focus is distinct from that of Gibbs et
...
(\(S \to \infty\)). My numerical results are entirely consistent with
this prediction (see Supplementary Information). Given that a complex
system is known to be stable (or unstable), \(Var(\gamma)\) is not
expected to increase or decrease the probability of stability. Rather,
component response rate variation increases the upper bound at which
...
results are broadly applicable to understanding stability of complex
networks across the physical, life, and social sciences.
...
finite \(S\). In simulations, the total variation in
\(\mathbf{M} = \mathbf{\gamma} \mathbf{A}\) row means that is
attributable to \(\mathbf{A}\) is small relative to that attributable to
\(\mathbf{\gamma}\), especially at high \(S\). Variation in
\(\mathbf{\gamma}\) specifically isolates the effects of differing
