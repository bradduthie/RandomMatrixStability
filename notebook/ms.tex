...
Given \(\sigma^{2}_\gamma = 0\), \(\Re(\lambda_{max})\) decrease
...
Ten thousand numerical simulations of \(\mathbf{M}\) revealed that
...
\(\Re(\lambda_{max})\) across \(\rho\) values when
\(\sigma^{2}_{\gamma} = 0\) versus \(\sigma^{2}_{\gamma} = 1/3\) given
\(S = 25\), \(C = 1\), and \(\sigma = 0.4\). For
\(-0.4 \leq \rho \leq 0.7\) (shaded region of Fig. 2), expected
\(\Re(\lambda_{max})\) was lower given \(\sigma^{2}_{\gamma} = 1/3\).
For \(\rho \geq -0.1\), the range of \(\Re(\lambda_{max})\) values
observed given \(\sigma^{2}_{\gamma} = 1/3\) also had a lower bound,
which included negative values for \(\rho = -0.1\) and \(\rho = 0\).
Hence, across a wide range of system correlations, variation in the
response rate of system components had a stabilising effect.

The stabilising effect of \(\sigma^{2}_{\gamma}\) across \(\rho\)
increased with increasing \(S\). Figure 3 shows 10000 numerical
simulations of \(\mathbf{M}\) for \(\sigma^{2}_{\gamma} = 0\) versus
\(\sigma^{2}_{\gamma} = 1/3\) across increasing \(S\) given \(C = 1\)
and \(\sigma = 0.2\) (\(\sigma\) has been lowered here to better
illustrate the effect of \(S\); note that now given \(S = 25\),
\(1 = \sigma\sqrt{SC}\)). For relatively small systems (\(S \leq 25\)),
positive \(\sigma^{2}_{\gamma}\) never decreased the expected
\(\Re(\lambda_{max})\). But as \(S\) increased, the curvilinear
relationship between \(\rho\) and \(\Re(\lambda_{max})\) resulted in
\(\mathbf{M}\) with lower expected \(\Re(\lambda_{max})\) for low
magnitudes of \(\rho\). In turn, as \(S\) increased, and systems became
more complex, positive \(\sigma^{2}_{\gamma}\) increased the proportion
of numerical simulations that were observed to be stable (see below).

\textbf{Simulation of random \(\mathbf{M}\) across \(\mathbf{S}\)}. To
investigate the effect of \(\sigma^{2}_{\gamma}\) on stability across
systems of varying complexity, I simulated random \(\mathbf{M}\)
matrices at \(\sigma = 0.4\) and \(C = 1\) across
\(S = \{2, 3, ..., 49, 50\}\). One million \(\mathbf{M}\) were simulated
for each \(S\), and the stability of \(\mathbf{M}\) was assessed given
\(\gamma = 1\) (\(\sigma^{2}_{\gamma} = 0\)) versus
\(\gamma \sim \mathcal{U}(0, 2)\) (\(\sigma^{2}_{\gamma} = 1/3\)). For
all \(S > 10\), I found that the number of stable random systems was
higher given \(\sigma^{2}_{\gamma} > 0\) than when
\(\sigma^{2}_{\gamma} = 0\) (Fig. 3; see Supplementary Information for
full table of results), and that the difference between the
probabilities of observing a stable system increased with an increase in
\(S\). In other words, the potential for \(\sigma^{2}_{\gamma}\) to
affect stability increased with system complexity and was most relevant
for systems on the cusp of being too complex to be realistically stable.
For the highest values of \(S\), nearly all systems that were stable
given varying \(\gamma\) would not have been stable given
\(\gamma = 1\).

I also simulated 100000 \(\mathbf{M}\) for three types of random
networks that are typically interpreted as differing interspecific
ecological interaction
types\textsuperscript{\protect\hyperlink{ref-Allesina2012}{2},\protect\hyperlink{ref-Allesina2011}{25}}.
These interactions types are competitive, mutualist, or predator-prey,
as modelled by off-diagonal elements of \(\mathbf{M}\) that are
constrained to be negative, positive, or paired such that if
\(A_{ij} > 0\) then \(A_{ji} < 0\),
respectively\textsuperscript{\protect\hyperlink{ref-Allesina2012}{2}}
(but are otherwise identical to the purely random \(\mathbf{M}\)). As
\(S\) increased, a higher number of stable systems were observed given
\(\sigma^{2}_{\gamma}>0\) versus \(\sigma^{2}_{\gamma}=0\) for
competitor and predator-prey, but not mutualist, systems. A higher
number of stable systems were observed whenever \(S > 12\) and
\(S > 40\) for competitive and predator-prey systems, respectively (note
that \(\rho < 0\) by definition for predator-prey systems, making
stability more likely overall). The stability of mutualist systems was
never affected by \(\sigma^{2}_{\gamma}\).

The effect of \(\sigma^{2}_{\gamma}\) on stability did not change
qualitatively across values of \(C\), \(\sigma\), or different
...
commonly observed network structures, I simulated 1 million
\(\textbf{M}\) for
small-world\textsuperscript{\protect\hyperlink{ref-Watts1998}{15}},
...
absence of an interaction between components \(i\) and \(j\) constrained
...
In scale-free networks, the number of components with which a focal
component interacts follows a power law; typically a few components have
many interactions while most components have few
...
\(\mathbf{M}\) for \(\sigma^{2}_{\gamma}>0\) compared to
\(\sigma^{2}_{\gamma}=0\). For random networks, positive
\(\sigma^{2}_{\gamma}\) increased stability given \(S > 10\)
(\(\sigma = 0.4\) and \(C = 1\)), and therefore complexity
\(\sigma \sqrt{SC} \gtrapprox 1.26\). This threshold of complexity was
...
predator-prey networks. Overall, network structure does not greatly
change the effect that \(\sigma^{2}_{\gamma}\) has on increasing the
upper bound of complexity within which stability might reasonably be
observed.
...
Feasibility was unaffected by \(\sigma^{2}_{\gamma}\) and instead
...
\(\sqrt{\sigma^{2}_{A}(1 + \sigma^{2}_{\gamma})S} \approx\) 1.12.
