...
\(S = 25\) and \(\sigma = 0.4\). For \(-0.4 \leq \rho \leq 0.7\) (shaded
region of Figure 2), expected \(\Re(\lambda_{max})\) was lower given
\(\sigma^{2}_{\gamma} = 1/3\). For \(\rho \geq -0.1\), the range of
\(\Re(\lambda_{max})\) values observed given
\(\sigma^{2}_{\gamma} = 1/3\) also had a lower bound, which included
negative values for \(\rho = -0.1\) and \(\rho = 0\). Hence, across a
...
\(\sigma^{2}_{\gamma} = 1/3\) across increasing \(S\) given
\(\sigma = 0.2\) (\(\sigma\) has been lowered here to better illustrate
the effect of \(S\); note that now given \(S = 25\),
...
\(\Re(\lambda_{max})\) demonstrated the potential for \(Var(\gamma)\) to
increase system stability. Across \(S = \{2, 3, ..., 39, 40\}\), sets of
\(\gamma\) values were found that resulted in stable systems with
probabilities that were up to four orders of magnitude higher than when
\(\gamma = 1\) (Fig. 4), meaning that stability could often be achieved
by manipulating \(S\) \(\gamma\) values rather than \(S \times S\)
