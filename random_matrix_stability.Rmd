...
**Analyses of dynamic systems using random matrices find that stability generally decreases with increasing system size. But in ecological communities, the magnitude of interaction effects will be highly dependent upon species generation time. The per capita effect of individuals from species with relatively short generation times will be lower than that of individuals from species with longer generation times. Interaction effects on are therefore expected to be non-random within species, with some species having consistently weaker or stronger effects on other species per capita. Here we show that by accounting for variation in generation time among species across three orders of magnitude, community stability is substantially increased. Further, the influence of variation in generation time becomes exponentially more important as community size increases. We suggest that variation in species generation time might be a key contributor to stability of large ecological communities.**

********************************************************************************

The above is a draft introductory paragraph or abstract. I believe that we could get [an article on the above](https://www.nature.com/natecolevol/about/content) clearly in the space of 3500, or maybe even 1500 words. As far as I've been able to find, such an article would be novel and interesting (and mathematically correct).
...
Here I will only consider $\boldsymbol{A}$ and focus on the stability of ecological interactions given variation in generation time. I will use the same technique of @Patel2018 for varying ecological and evolutionary time scales, but a bit differently. I assume that the response of a particular species' growth rate (rows in $\boldsymbol{A}$) is consistently affected by a value $\gamma_{i}$ (I am using $\gamma_{i}$ instead of $\varepsilon_{i}$ to avoid confusing generation time variance with ecological versus evolutionary rate difference). Higher values of $\gamma_{i}$ correspond to shorter absolute generation times and lower values correspond to longer generation times. The matrix below $\boldsymbol{A}$ for two species $i$ and $j$ with generation times $\gamma_{i}$ and $\gamma_{j}$, respectively.
...
I will first allow random values of $\gamma_{i}$ to vary from $1$ to $100$, modelling two orders of magnitude of generation times. I will compare the stability of two sets of random community matrices -- one in which all $\gamma = 1$ and another in which $\gamma$ varies.
...
while(iter > 0){
    ...
    gamval  <- runif(n = nn, min = 1, max = 100);
    eps_dat <- rep(x = gamval, times = nn);
    eps_mat <- matrix(data = gamval, nrow = nn, ncol = nn, byrow = FALSE);
    A2      <- A2 * eps_mat;
...
The number of stable random matrices when $\gamma$ does not vary was $13$, and the number of stable random matrices when it does was $48$. To further investigate, it is worth looking at **how the amount of variation in generation time (range of $\gamma$) in a community affects stability**. Below is some code to look at stability in random communities across different magnitudes of variance in generation time for communities with eight species.
...
The code below produces the following results showing the proportion of random communities that are stable as the variation in generation time increases in orders of magnitude (note that I'm using eight species, so the y-axis shows a very low proportion range).
...
plot(x = log10(mag_var[,1]), y = stabi_3, type = "b", lwd = 2, ylim = c(0, 0.0005),
     xlab = "LOG Generation time variation among species", ylab = "Proportion stable",
     cex.axis = 1.5, cex.lab = 1.5, lty = "solid");
```

**It looks like once the generation time varies by an order of magnitude of around 1000, there is little return in terms of stability.** To continue, we can simulate different sizes of communities with species numbers between 2 and 20 for identical generation times versus magnitudes of generation time variation of 1000.
...
The plot below shows the result for the proportion of stable communities when generation time varies (solid line, closed points) versus when generation time varies over two orders of magnitude (dotted line, open points).
...
This is interesting. **As species number increases, variation in generation time appears to have a stronger influence in stabilising random communities**. Assuming that I have done this correctly, this could be a fairly high impact result -- variation in generation time is important for community stability. I have not seen this take-home message in the literatur -- at least, not presented in this way. Again, I am not sure if any of this is something that I can prove mathematically, and I cannot decide whether the result is interesting enough to try to publish by itself or in tandem with the above. If not, it might be another blog post. 
...
Code to print the above is below.
