...
[Working draft of the manuscript](https://bradduthie.github.io/random_matrix_stability/ms.html)

********************************************************************************

**UPDATE 28 APR**:

I have made some more work on [the manuscript](https://bradduthie.github.io/random_matrix_stability/ms.html). The different distributions of $\gamma$ are now not the central focus. Instead, I want to focus more on the distribution of eigenvalues and allow a uniform distribution $\gamma \sim \mathcal{U}(0, 2)$. I want to show the eigenvalues plotted out and go into a bit more detail of what is actually driving these distributions. The figure below shows one plot **a** where $\gamma = E[\gamma] \approx 1$ and one plot **b** where $\gamma \sim \mathcal{U}(0, 2)$. The sum of the eigenvalues needs to equal the sum of the rows [@Allesina2012], so the sum of the eigenvalues is identical in each.
...
It's worth noting that the **b** in the figure above resembles the power law graph of @Allesina2015, which also has variation in the distribution of values due to the structure of connectivity ($C$). It's also worth looking at a what happens when only 1 $\gamma$ out of the 1000 total is different. The plot on below in subpanel **a** shows what happens when one $\gamma = 18$ and the rest are $\gamma = 1 - 17/999$ (causing $E[\gamma] = 1$). The plot in subpanel **b** shows what happens when one $\gamma = 1/18$ and the rest are $\gamma = 1 + (17/18)/999$ (also causing $E[\gamma] = 1$).
...
C_mat  <- matrix(data = C_dat, nrow = 1000, ncol = 1000);
A_mat  <- A_mat * C_mat;
#gamma1 <- c(4,   rep(1 - 1/333,  999));
#gamma2 <- c(1/4, rep(1 + 1/1332, 999));
gamma1 <- c(18,   rep(1 - 17/999,  999));
gamma2 <- c(1/18, rep(1 + (17/18)/999, 999));
#gamma1 <- c(40,   rep(1 - 13/333,  999));
#gamma2 <- c(1/40, rep(1 + (39/40)/999, 999));
mu_gam <- mean(gammas);
diag(A_mat) <- -1;
A0     <- gamma1 * A_mat;
A1     <- gamma2 * A_mat;
...
A1vec       <- A1vec[is.na(A1vec) == FALSE];
par(mfrow = c(1, 2), mar = c(0.5, 0.5, 0.5, 0.5), oma = c(5, 5, 0, 0));
plot(A0_r, A0_i, xlim = c(-20.5, 15.5), ylim = c(-16.5,15.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1);
vl <- seq(from = 0, to = 2*pi, by = 0.001);
x0 <- sqrt(1000) * sd(A0vec) * cos(vl) + mean(diag(A0));
y0 <- sqrt(1000) * sd(A0vec) * sin(vl);
x1 <- sqrt(1000) * sd(A1vec) * cos(vl) + mean(diag(A1));
y1 <- sqrt(1000) * sd(A1vec) * sin(vl);
text(x = -15.5, y = 16, labels = "a", cex = 2);
points(x = x0, y = y0, type = "l", lwd = 3);
points(x = x1, y = y1, type = "l", col = "red", lwd = 3, lty = "dashed");
plot(A1_r, A1_i, xlim = c(-20.5, 15.5), ylim = c(-16.5,15.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1, col = "red",
     yaxt = "n");
text(x = -15.5, y = 16, labels = "b", cex = 2);
points(x = x1, y = y1, type = "l", col = "red", lwd = 3)
points(x = x0, y = y0, type = "l", lwd = 3, lty = "dashed");
mtext(side = 1, "Real", outer = TRUE, line = 3, cex = 2);
mtext(side = 2, "Imaginary", outer = TRUE, line = 2.5, cex = 2);
...
This does not add a whole lot of insight into what is happening, although it does show a couple eigenvalues leaving the circle in **b**. In **a**, the lowest real eigenvalue is `r min(A0_r)` and the highest is `r max(A0_r)`, while in **b**, the lowest real eigenvalue is `r min(A1_r)` and the highest is `r max(A1_r)`. More revealing is to consider a situation where only two values of $\gamma$ exist: 0.05 and 1.95. By selecting half $\gamma$ to be each, it becomes a bit clearer to see what is going on. In the
...
A1vec       <- A1vec[is.na(A1vec) == FALSE];
par(mfrow = c(1, 2), mar = c(0.5, 0.5, 0.5, 0.5), oma = c(5, 5, 0, 0));
plot(A0_r, A0_i, xlim = c(-19.5, 15.5), ylim = c(-19.5,15.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1);
vl <- seq(from = 0, to = 2*pi, by = 0.001);
x0 <- sqrt(1000) * sd(A0vec) * cos(vl) + mean(diag(A0));
y0 <- sqrt(1000) * sd(A0vec) * sin(vl);
x1 <- sqrt(1000) * sd(A1vec) * cos(vl) + mean(diag(A1));
y1 <- sqrt(1000) * sd(A1vec) * sin(vl);
text(x = -16.5, y = 14, labels = "a", cex = 2);
abline(v = 0, col = "blue");
points(x = x0, y = y0, type = "l", lwd = 3);
points(x = x1, y = y1, type = "l", col = "red", lwd = 3, lty = "dashed");
plot(A1_r, A1_i, xlim = c(-19.5, 15.5), ylim = c(-19.5,15.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1, col = "red",
     yaxt = "n");
text(x = -16.5, y = 14, labels = "b", cex = 2);
abline(v = 0, col = "blue");
points(x = x1, y = y1, type = "l", col = "red", lwd = 3)
points(x = x0, y = y0, type = "l", lwd = 3, lty = "dashed");
mtext(side = 1, "Real", outer = TRUE, line = 3, cex = 2);
...
I have added the blue vertical line where the x-axis equals zero. In **a**, the lowest real eigenvalue is `r min(A0_r)` and the highest is `r max(A0_r)`, while in **b**, the lowest real eigenvalue is `r min(A1_r)` and the highest is `r max(A1_r)`. But note the clump in **b**. Instead of being uniform, there are low values and high values. Overall, there are `r sum(A0_r < 0)` eigenvalues in **a** where the real component is less than zero, and there are `r sum(A1_r < 0)` eigenvalues in **b** where the real component is less than zero. Overall, because the mean is forced such that $E[\Re{(\lambda_{1})}] = 1$ (the mean on the x-axis is exactly -1 in both panels), the smaller circle on the inside created where the subset of $\gamma$ values is very low has a small probability of pulling real eigenvalues to the left of 0, and in smaller systems (lower $S$), these eigenvalues can be the leading one, thereby stabilising the system.

Note that the sum of the real components of all eigenvalues needs to equal the sum of the diagonal of a matrix [@Allesina2015]. So we can break the two haves of the matrix represented by **b** above into the upper half where the sum equals `r sum(diag(A1[1:500, 1:500]))`, and the lower half where the sum equals `r sum(diag(A1[501:1000, 501:1000]))`. We can then partition the eigenvalues in each half, $\lambda_{upper}$ and $\lambda_{lower}$. 

We can also note that the values of the off diagonals in the upper half of the matrix are multiplied by 1.95, so $\sigma_{upper} = \sigma(1.95) = 1.95$ and $\sigma_{lower} = \sigma(0.05) = 0.05$. This effectively creates two circles, one with a radius of $1.95\sqrt{SC}$; this is shown explicitly below with the red circles.
...
A1vec       <- A1vec[is.na(A1vec) == FALSE];
par(mfrow = c(1, 2), mar = c(0.5, 0.5, 0.5, 0.5), oma = c(5, 5, 0, 0));
plot(A0_r, A0_i, xlim = c(-19.5, 16.5), ylim = c(-19.5,16.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1);
vl <- seq(from = 0, to = 2*pi, by = 0.001);
x0 <- sqrt(1000) * sd(A0vec) * cos(vl) + mean(diag(A0));
y0 <- sqrt(1000) * sd(A0vec) * sin(vl);
...
points(x = x0, y = y0, type = "l", lwd = 3);
points(x = x1, y = y1, type = "l", col = "red", lwd = 3, lty = "dashed");
plot(A1_r, A1_i, xlim = c(-19.5, 16.5), ylim = c(-19.5,16.5), pch = 4, cex = 0.7,
     xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 1.5, asp = 1, col = "red",
     yaxt = "n");
A1_vm       <- A1;
diag(A1_vm) <- NA;
A1vecU      <- as.vector(A1_vm[1:500,]);
A1vecU      <- A1vecU[is.na(A1vecU) == FALSE];
A1vecL      <- as.vector(A1_vm[501:1000,]);
A1vecL      <- A1vecL[is.na(A1vecL) == FALSE];
x0 <- sqrt(500) * sd(A1vecU) * cos(vl) + mean(diag(A1[1:500,1:500]));
y0 <- sqrt(500) * sd(A1vecU) * sin(vl);
x1 <- sqrt(500) * sd(A1vecL) * cos(vl) + mean(diag(A1[501:1000,501:1000]));
y1 <- sqrt(500) * sd(A1vecL ) * sin(vl);
text(x = -16.5, y = 14, labels = "b", cex = 2);
abline(v = 0, col = "blue");
points(x = x1, y = y1, type = "l", col = "orange", lwd = 2)
points(x = x0, y = y0, type = "l", lwd = 2, lty = "solid", col = "orange");

A1_vm       <- A1;
diag(A1_vm) <- NA;
A1vec       <- as.vector(A1_vm);
A1vec       <- A1vec[is.na(A1vec) == FALSE];
x3 <- sqrt(1000) * sd(A1vec) * cos(vl) + mean(diag(A1));
y3 <- sqrt(1000) * sd(A1vec) * sin(vl);
points(x = x3, y = y3, type = "l", col = "red", lwd = 3, lty = "dashed");

mtext(side = 1, "Real", outer = TRUE, line = 3, cex = 2);
mtext(side = 2, "Imaginary", outer = TRUE, line = 2.5, cex = 2);
```

Note that the large orange is pushed leftward (red dashed circles are identical), though I am not entirely sure why yet. It should be noted that the upper-left and lower-right squares of the matrix should be partitionable. The remaining squares have off-diagonals pulled from matching distributions, so we should really be able to think of the whole thing as a combination of the upper and lower rows, I think. 

Also note how the smaller circle is pulled rightwards (centre = `r mean(x0)`) while the larger circle is pulled leftwards (centre = `r mean(x1)`), while the overall centre remains the same (red dashed circle, centre = `r mean(x3)`).

**UPDATE 8 APR**:

**New result -- a correlation between the number of positive or negative values in a row of the Jacobian matrix (e.g., corresponding to a species that is generally positively affected by increases in other species -- generalist predator, or generally negatively affected by increases in other species -- a producer or resource), and the response rate of that row (e.g., species generation), is *highly* stabilising, and I believe that I can dig into the matrices and show that *this* is the mechanism that is tweaked when $Var(\gamma_{i})$ is included. In other words, variation in generation time inevitably increases or decreases the correlation (from zero) between sum of row signs and *types* of interactions and generation time -- which I will now generalise to something like reaction rate.** I will present these results in figures soon. I will also show that 1. general variation in $\gamma_{i}$ changes the correlation between sign counts and generation rate and 2. when this correlation is higher or lower, eigen values are more likely to all be negative.
...
The above is saved as `random_pos.csv`. The rate of change $\gamma_{i}$ in row $i$ equals the sum values in the row, plus the minimum number of values (e.g., four positive and three negative value equals one, plus whatever is the minimum -- if -3, then the end value should be four).
...
The above is saved as `random_neg.csv`. The rate of change $\gamma_{i}$ in row $i$ equals the opposite of the sum values in the row, plus the minimum number of values.
...
random      <- read.csv(file = "sim_results_old/oldest/random_community.csv");
competition <- read.csv(file = "sim_results_old/oldest/competitor_community.csv");
mutualism   <- read.csv(file = "sim_results_old/oldest/mutualist_community.csv");
pred_prey   <- read.csv(file = "sim_results_old/oldest/predator_prey_community.csv");
# ==============================================================================
par(mfrow = c(2, 2));
# ==============================================================================
dat      <- random;
...
```{r, echo = FALSE}
library(knitr);
dat  <- read.table(file = "sim_results_old/oldest/mat_dat.csv", 
                   header = TRUE, sep = ",");
...
mag_var <- read.table(file = "sim_results_old/oldest/var_gen_mag.csv", 
                      header = TRUE, sep = ",");
...
<a name="ms">The manuscript draft</a>

********************************************************************************

**The stability of a complex system generally decreases with increasing system size, as is demonstrated by random matrix theory [@May1972; @Allesina2012]. This counter-intuitive result, first shown by May [@May1972], is broadly relevant for understanding the dynamics and persistence of systems such as chemical, biological, and social networks. Much attention has especially been given to the stability of ecological communities such as food webs or mutualist networks, with recent work investigating how different community structures affect stability [@Allesina2012]. But more broadly, stabilising mechanisms in complex systems remain under-explored, and the effect of variation in the response rate of individual system components remains unexplored. Here I show that when components of a complex system respond to system dynamics at different rates, system stability is strongly increased. This increase in stability is driven by the correlation between a focal component's response rate and the number of other components in the system that have a negative or positive effect on it. When variation in component response rate ($\gamma$) is included in random interaction matrices, the effect of such emerging correlations necessarily increases stability. Further, these effects become increasingly more important as system size increases. My results therefore reveal a previously unconsidered driver of system stability that is likely to be pervasive across all complex systems. Future research in complex systems should therefore account for the varying response rates of individual system components when assessing whole system stability.** 
